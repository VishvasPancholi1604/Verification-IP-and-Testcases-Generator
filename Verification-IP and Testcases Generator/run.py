...
# compilation and simulation script core
# returns the script with project name and
# required paths automatically set up so user
# doesn't have to manually do it..
# NOTE:
    # The script supports Mentor's QuestaSim and Synopsys VCS tools
    # not currently tased with Synopsys VCS
def create_run_file(project_name):
    current_working_dir = os.getcwd()
    project_folder_path = os.path.join(current_working_dir, f'ei_{project_name}')
    dev_folder_path = os.path.join(project_folder_path, 'DEVELOPMENT')
    sim_dir_path = os.path.join(dev_folder_path, 'SIM')
    test_dir_path = os.path.join(dev_folder_path, 'TEST')

    run_file = r'''import os, argparse, subprocess, time, json
...
f"""
# setting up paths
working_sim_directory = r'{sim_dir_path}'
testcase_directory    = r'{test_dir_path}'
...
questa_commands = {
''' + \
f"""    'compile'  : 'vlog ..\SRC\ei_{project_name}_package.sv ..\TOP\ei_{project_name}_top.sv +incdir+..\SRC +incdir+..\ENV +incdir+..\TEST',""" + \
    r"""
